...
const FormPage = () => {
	...
	const _handleSubmit = async (event) => {
		...
		if (!validName.test(firstName) || !validName.test(lastName)) {
			setError("*Your name must contain letters only and at least two letters*");
			return;
		}
		...
		<div className="formpage">
			...
			<form onSubmit={_handleSubmit}>
				...
				<label className="infolabel3">
					Email address:
					<input className="infoinput" required type="text" placeholder="Enter email" onChange={(e) => setEmail(e.target.value)} value={email} />
				</label>
